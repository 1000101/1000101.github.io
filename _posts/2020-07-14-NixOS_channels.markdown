...
comments: true
---
Alright, now that we're running NixOS, it's time to go down the rabbit hole! Ready? In this episode, I'll try to explain the parts which make NixOS so unique and take a slight glance of how it works under the hood. First off, we'll take a look at channels.
...
What this essentially means is, that you get both the benefits of rapid development (since the code from which the channel is created is directly on GitHub) combined with stability (since there are tests that need to be passed first, for the channel to get published/updated).
...
[Here's][link-status] a handy site for checking the current state of channels. We'll discuss the active channels displayed here in the next section:
...
Simply put, channels are used for package installation and upgrades. In NixOS, regular user and system user (root) are strictly separated. That's why we need to talk about both user and system (root) channels. User channels are not mandatory and they can be used when installing software through `nix-env`. Notice that we don't have to use `sudo` as with other Linux distros. That's because we're not manipulating with the system channel.
...
$ nix-info -m
 ...
 - sandbox: `yes`
 - version: `nix-env (Nix) 2.3.6`
 <b>- channels(b1000101): `"unstable-20.09pre233323.dc80d7bc4a2`</b>
 - channels(root): `"nixos-20.03.2411.30fb4e1e206"`
 - nixpkgs: `/nix/var/nix/profiles/per-user/root/channels/nixos`
</pre>
...
$ nix-info -m
 ...
 - sandbox: `yes`
 - version: `nix-env (Nix) 2.3.6`
 - channels(b1000101): `"unstable-20.09pre<b>234194.8d05772134f</b>"`
 - channels(root): `"nixos-20.03.2411.30fb4e1e206"`
 - nixpkgs: `/nix/var/nix/profiles/per-user/root/channels/nixos`
</pre>
...
$ nix-info -m
 ...
 <b>- channels(root): `"nixos-20.03.2411.30fb4e1e206"`</b>
 - nixpkgs: `/nix/var/nix/profiles/per-user/root/channels/nixos`
</pre>
...
$ nix-info -m
 ...
 - version: `nix-env (Nix) 2.3.6`
 - channels(b1000101): `"unstable-20.09pre234194.8d05772134f"`
 <b>- channels(root): `"nixos-20.03.2520.add5529b3ee"`</b>
 - nixpkgs: `/nix/var/nix/profiles/per-user/root/channels/nixos`
</pre>
